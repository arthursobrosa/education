...
extension FocusSessionSettingsViewController {
    func createToggle(for indexPath: IndexPath) -> UISwitch {
        ...
        switch section {
            case 0:
                ...
                if let selectedData = self.viewModel.subjectsNames.first(where: { $0 == self.viewModel.subjectsNames[row - 1] }) {
                    isOn = selectedData == self.viewModel.selectedSubjectName
                }
            case 1:
    ...
    @objc func subjectToggleSwitched(_ sender: UISwitch) {
        ...
        self.viewModel.selectedSubjectName = self.viewModel.subjectsNames[(sender.isOn ? sender.tag : 0)]
    }
    
    func unselectedToggles(_ tag: Int) {
        for i in 0..<self.viewModel.subjectsNames.count {
    ...
    func selectNoneToggle() {
        ...
        let toggle = cell?.accessoryView as! UISwitch
        toggle.isOn = true
        self.viewModel.selectedSubjectName = self.viewModel.subjectsNames[toggle.tag]
    }
}
